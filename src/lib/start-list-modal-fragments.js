...
export const build_start_list_modal_list_html = (startListInput, eventId = '', startListTotal = null) => {
  ...
  const athletes = mainList.map((athlete) => {
    ...
    const instagramLink = hasInstagram
      ? `<a class="w-9 h-9 flex items-center justify-center rounded-lg bg-surface-container-high hover:bg-surface-bright transition-colors" href="${escape_html(`https://www.instagram.com/${encodeURIComponent(String(instagram))}/`)}" target="_blank" rel="noopener" onclick="window.gtag && gtag('event', 'start_list_instagram_clicked')">
    ...
    const athleteProfile = hasAthleteId || hasInstagram
      ? `<div class="flex items-center gap-3">
          ${instagramLink}
          ${hasAthleteId ? `<button class="bg-primary-container text-on-primary-fixed px-4 py-2 rounded-lg font-headline font-bold text-xs tracking-tight hover:brightness-110 active:scale-95 transition-all" onclick="window.gtag && gtag('event', 'start_list_ifsc_profile_clicked'); window.open('${escape_html(`https://ifsc.results.info/athlete/${encodeURIComponent(String(athleteId))}`)}', '_blank')"><span class="hidden lg:inline">IFSC </span>PROFILE</button>` : ''}
        </div>`
      : '';
