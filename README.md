...
- Season pages are prerendered at `/season/:season` (for example `/season/2026`).
- Event pages are prerendered at `/season/:season/event/:eventId`.
- Event and season JSON is injected at build-time, so season pages no longer fetch `events_*.json` via AJAX.
...
Playwright is used for browser-level integration tests in [`tests/integration`](tests/integration).

- `make test` runs the suite in Docker (recommended, no local Node setup required).
